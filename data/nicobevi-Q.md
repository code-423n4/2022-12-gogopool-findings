# [LOW] - Reconsider the storage implementation
...
Because all the storage used across the entire protocol (or the biggest part of the storage at least) is centralized in a single contract it implies that every storage reading has to be made through external calls. This requirement adds a lot of gas consumption to every function execution that requires any storage information. I understand that it's almost impossible to improve this without changing the entire protocol implementation. But I found that it'd be worth to mention here.

### Error prone keys implementation

Something that I thing could be improved a lot without major changes to the protocol's functionality is the way that the slot keys are implemented.

There're a lot of duplicated code across the contracts that calculates the same keys for the same storage slots. This is a big security concern since it requires just a misspelled character on one of this implementations to break the functionality silently (mostly) or hardly to detect at least.
...
This calculates the key used for the initial amount for that particular minipool (through the `minipoolIndex` variable), the problem is that we have that same keccak256 calculation on multiple places and something like this will return a completely different key:

```solidity
  keccak256(abi.encodePacked("minipool.item", minipoolIndex, "avaxNodeOpInitialAmt");
```

But the bug is really difficult to see (the issue is just one single character).
...
On that way, we can have a centralized point referencing all the keys needed for the protocol and be sure that there is no duplication or errors.
...
I'm a little concern about the amount of extra code required just to be able to work with this storage implementation. But I'm aware that, just like for the first point, it would be difficult to change this (if not impossible) without big changes to the protocol's implementation.

### Limited for custom types (structs)

Something that I do think could be improved is the limitations that we have for custom types.

For example, on the `MinipoolManager.sol` implementation we found that we have this struct defined:
...
This could be improved a lot just adding a custom mapping on storage (with some custom helpers) using that custom struct that we have defined on the contract already.
